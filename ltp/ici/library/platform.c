...
static int	itcpSendBytes(int *sock, char *from, int length)
{
	...
	while (1)	/*	Continue until not interrupted.		*/
	{
		...
		bytesWritten = send(*sock, from, length, 0);
		if (bytesWritten < 0)
		{
			switch (errno)
			{
			...
			putSysErrmsg("send error on TCP socket", itoa(*sock));
		}
...
int	itcp_recv(int *sock, char *into, int length)
{
	...
	while (bytesToRecv > 0)
	{
		...
		bytesRead = recv(*sock, into, bytesToRecv, 0);
		switch (bytesRead)
		{
		case -1:
			switch (errno)
			...
			default:
				putSysErrmsg("recv() error on TCP socket",
						NULL);
				return bytesRead;
			}
