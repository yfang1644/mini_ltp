...
int	ltpei_add_xmit_header_extension(LtpXmitSeg *segment,
			char tag, int valueLength, char *value);
int	ltpei_add_xmit_trailer_extension(LtpXmitSeg *segment,
			char tag, int valueLength, char *value);

int	ltpei_parse_extension(char **cursor, int *bytesRemaining,
			Lyst extensionsList, unsigned int *offset);
int	ltpei_record_extensions(LtpPdu *pdu, Lyst headerExtensions,
			Lyst trailerExtensions);
void	ltpei_discard_extensions(Lyst extensions);

void	ltpei_destroy_extension(Sdr, Object, void *);
...
*/
int	invokeInboundBeforeContentProcessingCallbacks
			(LtpRecvSeg *segment, Lyst headerExtensions,
			 Lyst trailerExtensions, char *segmentRawData,
			 LtpVspan *vspan);
/*
...
- -1 if the callback needs to call off the segment processing
*/
int	invokeOutboundOnHeaderExtensionGenerationCallbacks
			(LtpXmitSeg *segment);
/*
...
- -1 if the callback needs to call off the segment processing
*/
int	invokeOutboundOnTrailerExtensionGenerationCallbacks
			(LtpXmitSeg *segment);
